    ...
    return (
        <div className="flex items-center justify-start gap-4 px-6">
            {tabs.map((tab) => (
                <button
                    key={tab.id}
                    className={`flex items-center space-x-2 border-b-2 px-2 py-4 text-sm font-medium transition-all ${
                        activeTab === tab.id
                            ? 'border-stem-green-500 text-stem-green-500'
                            : 'border-transparent text-noble-300 hover:text-stem-green-500'
                    }`}
